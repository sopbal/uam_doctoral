...
"""
Calculate a p-values t-test checking if an HMM is overrepresented in a selected set of sequences
(e.g. lytic enzymes, compared to the provided background - e.g. all UniRef90 proteins)
"""
...
class HmmTtest:
    ...
    def __init__(self,
        ...
        for HMM overrepresentation in the positive dataset and
        writing the results as a xlsx spreadsheet.
        ...
        :param threshold: minimal number of protein hits in the positive dataset to consider
        """
    ...
    def load_score_dict(inputfile,
        ...
        and filter out hmms that don't align to at least
        {threshold} proteins.
...
if __name__ == '__main__':
    ...
    pos_fasta = Path(' ... /EnzyBase.fasta')  # path to protein fasta that was used to generate positive hits
    bbgn_fasta = Path(' ... /UniRef90.faa')  # path to protein fasta that was used to generate background hits
    descriptions = Path(' ... /totaldescription.pkl')  # path to protein fasta that was used to generate background hits

    output = Path(' ... /studentttest_results.xlsx')
